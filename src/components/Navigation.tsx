...
const StatusIndicator = ({ label, active = true }: { label: string, active?: boolean }) => (
  <div className="flex items-center gap-2 px-2 py-1 rounded-full bg-white/5 border border-white/5 text-[10px] font-mono text-gray-400">
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 flex items-center gap-2">
        <link.icon className={`text-sm transition-colors duration-300 ${isActive ? 'text-cyan-400' : 'text-gray-400 group-hover:text-white'}`} />
        <span className={`text-xs font-medium uppercase tracking-wider transition-colors duration-300 ${isActive ? 'text-white' : 'text-gray-400 group-hover:text-white'}`}>
          {link.label}
        </span>
      </div>
      ...
      <header className="fixed top-0 left-0 right-0 z-50 flex justify-center pt-4 px-4">
        <motion.div
          ...
          className={`
            ...
            ${scrolled 
              ? 'bg-[#050505]/80 backdrop-blur-xl border border-white/10 shadow-2xl shadow-black/50 rounded-2xl p-2' 
          ...
          <div className="flex items-center justify-between relative">
            ...
            <div className="flex items-center gap-6">
              <Link href="/" className="group flex items-center gap-3 relative z-20">
                <div className="relative w-10 h-10 flex items-center justify-center overflow-hidden rounded-xl bg-gradient-to-br from-white/10 to-white/5 border border-white/10 group-hover:border-cyan-500/50 transition-all duration-500 group-hover:shadow-[0_0_20px_rgba(6,182,212,0.3)]">
                  <div className="absolute inset-0 bg-cyan-500/20 opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
                  <FaBrain className="text-gray-300 group-hover:text-cyan-400 transition-colors duration-300 text-lg relative z-10" />
                </div>
                <div className="hidden sm:flex flex-col">
                  ...
                  <span className="text-[10px] text-gray-500 font-mono group-hover:text-cyan-500/70 transition-colors">
            ...
            <nav className="hidden lg:flex items-center absolute left-1/2 -translate-x-1/2">
              <div className="flex items-center gap-1 p-1 bg-white/5 border border-white/5 rounded-xl backdrop-blur-md">
                {NAV_LINKS.map((link) => (
                  <NavItem
                    key={link.id}
                    link={link}
            ...
            <div className="flex items-center gap-3">
              {/* Time Display */}
              <div className="hidden md:block text-xs font-mono text-gray-400 bg-white/5 px-3 py-1.5 rounded-lg border border-white/5">
              ...
              <button
                ...
                className="lg:hidden p-2.5 text-gray-400 hover:text-white bg-white/5 border border-white/10 rounded-xl transition-colors"
              >
      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-40 bg-[#050505]/95 backdrop-blur-2xl lg:hidden flex flex-col pt-32 px-6"
          >
            {/* Background Grid */}
                ...
                >
                  ...
                  >
                    <div className="flex items-center gap-4">
                      <div className={`p-2 rounded-lg ${pathname === link.href ? 'bg-cyan-500/20 text-cyan-400' : 'bg-black/20 text-gray-500 group-hover:text-white'}`}>
                        <link.icon size={20} />
                      </div>
