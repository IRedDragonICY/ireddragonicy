  ...
  return (
    ...
    >
        <motion.div 
            ...
            className="relative w-full max-w-4xl bg-card border border-cyan-500/30 rounded-xl overflow-hidden shadow-[0_0_50px_rgba(34,211,238,0.15)] flex flex-col md:flex-row"
            onClick={(e) => e.stopPropagation()}
        >
            {/* Close Button */}
            <button 
                onClick={onClose}
                className="absolute top-4 right-4 z-50 w-8 h-8 flex items-center justify-center rounded-full bg-background/50 border border-foreground/10 text-muted-foreground hover:text-foreground hover:bg-red-500/20 hover:border-red-500/50 transition-all"
            >
                <FaTimes size={14} />
            </button>

            {/* Left: Visual/Image Section */}
            <div className="w-full md:w-2/5 bg-background relative overflow-hidden group border-b md:border-b-0 md:border-r border-foreground/10 p-8 flex items-center justify-center">
                
                {/* Background Grid */}
                <div className="absolute inset-0 bg-[url('/grid-pattern.svg')] opacity-20 invert dark:invert-0" />
            ...
            <div className="flex-1 bg-card p-6 md:p-8 flex flex-col">
                
                {/* Header */}
                <div className="mb-6">
                    <div className="flex items-center gap-3 mb-2">
                        <FaGoogle className="text-muted-foreground text-xl" />
                ...
                <div className="flex-1 space-y-6">
                        ...
                        <div className="grid grid-cols-1 gap-3">
                            {skills.map((skill, i) => (
                                <div key={i} className="group">
                                    <div className="flex justify-between text-xs font-mono text-muted-foreground mb-1 group-hover:text-foreground transition-colors">
                                        <span>{skill}</span>
                    ...
                    {/* Terminal Output */}
                    <div className="bg-background/50 rounded-lg border border-foreground/5 p-3 font-mono text-[10px] h-32 overflow-y-auto custom-scrollbar relative">
                            ...
                            {terminalLogs.map((log, i) => (
                                log ? (
                                    <div key={i} className="flex gap-2">
                                        ...
                                        <span className={log.includes('VALID') ? 'text-green-500' : 'text-muted-foreground'}>
                                            {log}
                                        </span>
                                    </div>
                                ) : null
                ...
                {/* Footer Actions */}
                <div className="mt-8 pt-6 border-t border-foreground/10 flex items-center justify-between gap-4">
                    <div className="flex flex-col">
                         <span className="text-[9px] text-muted-foreground font-mono uppercase">Verification_Hash</span>
                         <span className="text-xs font-mono text-foreground truncate max-w-[150px] opacity-50">
